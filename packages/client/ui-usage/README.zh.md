...
- 今天 / 7天 / 30天 区间切换（默认选「今天」，对应一天窗口）。「今天」按小时渲染柱子，多天窗口按天渲染。
- 每个 token 类别（输入、缓存命中、输出）一张指标卡，外加一张请求次数卡，展示各自的窗口总量。
- 堆叠柱状图，带 Y 轴网格线、X 轴和图例；悬浮到某个柱子会弹出该柱的精确数值。靠边缘的柱子会让悬浮框向内对齐，避免被面板裁剪。
