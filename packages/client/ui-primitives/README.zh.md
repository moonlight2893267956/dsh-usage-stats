...
- **字形级图标是重新绘制的近似版本**：鱼形标志、ui-settings-general 持有的图表字形，以及 ui-conversation 持有的闪光图标均为手工重建版本——鱼形与闪光图标代替本地设计数据无法导出矢量几何的字体字形，图表字形则因 deepsuite 图标集未提供图表导出而用于逐期用量趋势；三者均待精确导出路径就绪后替换。
